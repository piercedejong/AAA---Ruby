...
$(document).ready(function() {
  ...
  // battle("land", 1, -1);
  // JQuery Function called from the the battle simulation button being pressed
  $("#battle").click(function(e) {
    // function battle(type, battles, round) {
    console.clear()
    aWins = 0
    draws = 0
    dWins = 0
    ...
    // Get the unit arrays for battle that user specified
    var units
    if (type == "land")
      units = getLand()
    else if (type == "ocean")
      ...
      log("Error: Please choose either land or ocean")
    // Loop through the specified amount of battles and run a simulation for each
    var i;
    for (i = 0; i < totalBattles; i++) {
      rounds = -1

      // Reset arrays of units for next Simulationn
      aUnits = jQuery.extend(true, {}, units.attack)
      dUnits = jQuery.extend(true, {}, units.defense)

      // Get the total amount of units for each side
      getTotal()
      // Display the total amount of each unit type
      // If only one simulation is runninig (totalBattles ==1)
      print("Starting Units")
      printUnits();
      print("Attacker IPC: " + getCost(aUnits))
      print("Defender IPC: " + getCost(dUnits))
      print("")
      ...
      // If both sides have no units left it is a draw
      if (aTotal <= 0 && dTotal <= 0) {
        draws += 1
        // If the attacker has units left, attacker wins
      } else if (aTotal > 0) {
        aWins += 1
      } else { // Defenser won
        dWins += 1
    ...
    // Print out the results of the simualtion
    var a = aWins / totalBattles * 100
    var b = draws / totalBattles * 100
    var c = dWins / totalBattles * 100
    var x = parseFloat(Math.round(a * 100) / 100).toFixed(2);
    var y = parseFloat(Math.round(b * 100) / 100).toFixed(2);
    var z = parseFloat(Math.round(c * 100) / 100).toFixed(2);

    log("********END RESULT********")
    printUnits();

    log("Attacker Win Percent: " + x)
    log("Defender Win Percent: " + z);
    log("Draw Percent: " + y)
  })

  function symBattle(type) {
    while ((aTotal > 0 && dTotal > 0) && rounds != 0) {
      print("*******NEXT ROUND*******")
      printUnits();
      ...
      if (type == "land") {
        ...
        log("Error: incorrect battle type");
      }
      getTotal();
      rounds -= 1
    }
  ...
  function oceanBattle() {
    ...
    // Get the amount of hits casued by both the attacker and defnder naval units
    // against enemny aircraft and remove those units
    aaa = oceanAAA(aUnits, dUnits);
    aUnits = oceanDamageAirAttacker(aUnits, aaa.dHits);
    dUnits = oceanDamageAirDefender(dUnits, aaa.aHits);

    // Get the total amount of suprise strikes caused by submarines
    // For both attacker and defender
    // Remove casulaties from both sides before they can return fire
    subs = oceanSub(aUnits, dUnits);
    aUnits = oceanDamageNaval(aUnits, subs.dSpclHits)
    dUnits = oceanDamageNaval(dUnits, subs.aSpclHits)

    // Get the hits for both side for all naval units
    // Exculde the subs that fired in the suprise attack phase
    aHitsNaval = oceanHitsNavalAttacker(aUnits, subs.aSpcl)
    dHitsNaval = oceanHitsNavalDefender(dUnits, subs.dSpcl)

    // Get the hits for both side for all air units
    aHitsAir = oceanHitsAirAttacker(aUnits)
    dHitsAir = oceanHitsAirDefender(dUnits)

    // Take damage on naval units from hits of enemy naval units
    aUnits = oceanDamageNaval(aUnits, dHitsNaval)
    dUnits = oceanDamageNaval(dUnits, aHitsNaval)

    // Take damage on ALL units from hits of enenmy aircraft
    aUnits = oceanDamageAttacker(aUnits, dHitsAir)
    dUnits = oceanDamageDefender(dUnits, aHitsAir)
  }

  function oceanSub(aUnits, dUnits) {
    var aSpclHits = 0
    var aSpcl = 0
    if (aUnits.sub > dUnits.dst) {
      aSpcl = aUnits.sub - dUnits.dst
      aSpclHits = calcUnit(aSpcl, 4, "Attacker Sub Sneak Hits")
    }

    var dSpclHits = 0
    var dSpcl = 0
    if (dUnits.sub > aUnits.dst) {
      dSpcl = dUnits.sub - dUnits.dst
      dSpclHits = calcUnit(dSpcl, 2, "Defender Sub Sneak Hits"); // Subs not stopped by dst
    }
  ...
  // Get the total amount of AAA hits casue by both the attacker
  // and defnde naval units against enemeny aircraft
  // No damge or loses are taken in this function
  // Return aHits - int amount of hits the attacker AAA caused
  // 				dHits - int amount of hits the defender AAA caused
  function oceanAAA(aUnits, dUnits) {
    var aAir = aUnits.fgh + aUnits.tac + aUnits.bmb
    var dAir = dUnits.fgh + dUnits.tac + dUnits.bmb

    var aNvl = aUnits.dst + aUnits.crs * 2 + aUnits.btl * 3 + aUnits.dbt * 2
    var dNvl = dUnits.dst + dUnits.crs * 2 + dUnits.btl * 3 + dUnits.dbt * 2

    var aHits = 0
    if (aNvl >= dAir) {
      aHits = calcUnit(dAir, 2, "Attacker AAA Hits");
    } else {
      aHits = calcUnit(aNvl, 2, "Attacker AAA Hits");
    }
    var dHits = 0
    if (dNvl >= aAir) {
      dHits = calcUnit(aAir, 2, "Defender AAA Hits");
    } else {
      dHits = calcUnit(dNvl, 2, "Defender AAA Hits");
    }
    print(" ")
  ...
  // All attacker planes that were hit by aa before the round of combat
  // hits are from the AAA of the defnder naval ships
  function oceanDamageAirAttacker(units, hits) {
    while (hits > 0) {
      if (units.fgh > 0) {
        units.fgh -= 1
      } else if (units.tac > 0) {
  ...
  function oceanDamageAirDefender(units, hits) {
    while (hits > 0) {
      if (units.tac > 0) {
        units.tac -= 1
      } else if (units.fgh > 0) {
  ...
  function oceanDamageNaval(units, hits) {
    while (hits > 0) {
      if (units.btl > 0) {
        units.btl -= 1
        units.dbt += 1
  ...
  function oceanDamageAttacker(units, hits) {
    while (hits > 0) {
      if (units.btl > 0) {
        units.btl -= 1
        units.dbt += 1
  ...
  function oceanDamageDefender(units, hits) {
    while (hits > 0) {
      if (units.btl > 0) {
        units.btl -= 1
        units.dbt += 1
  ...
  function oceanHitsAirAttacker(units) {
    var hits = 0
    hits += calcUnit(units.fgh, 6, "Fgh")
    hits += calcUnit(units.tac, 7, "Tac")
    hits += calcUnit(units.bmb, 8, "Bmb")
    print("Attacker Air Hits: " + hits)
    print(" ")
    return hits
  }

  function oceanHitsAirDefender(units) {
    var hits = 0
    hits += calcUnit(units.fgh, 6, "Fgh")
    hits += calcUnit(units.tac, 5, "Tac")
    print("Defender Air Hits: " + hits)
    print(" ")
    return hits
  }

  function oceanHitsNavalAttacker(units, spcl) {
    var hits = 0
    hits += calcUnit(units.sub - spcl, 4, "Sub")
    hits += calcUnit(units.dst, 4, "Dst")
    hits += calcUnit(units.crs, 6, "Crs")
    hits += calcUnit(units.btl, 9, "Btl")
    hits += calcUnit(units.dbt, 7, "DBt")
    print("Attacker Naval Hits: " + hits)
    print(" ")
    return hits
  }

  function oceanHitsNavalDefender(units, spcl) {
    var hits = 0
    hits += calcUnit(units.sub - spcl, 2, "Sub")
    hits += calcUnit(units.dst, 4, "Dst")
    hits += calcUnit(units.crs, 6, "Crs")
    hits += calcUnit(units.btl, 8, "Btl")
    hits += calcUnit(units.dbt, 6, "DBt")
    print("Defender Naval Hits: " + hits)
    print(" ")
    return hits
  }
  ...
  function landBattle() {
    ...
    boi = landFgh(aUnits, dUnits);
    aUnits = boi.aUnits
    dUnits = boi.dUnits

    aHits = landHitsAttacker(aUnits, aLand);
    dHits = landHitsDefender(dUnits, dLand);

    aUnits = landDamageAttacker(aUnits, dHits);
    dUnits = landDamageDefender(dUnits, aHits);
  }

  function landFgh(aUnits, dUnits) {
    if (aUnits.fgh > 0 && dUnits.fgh > 0) {
      var a = calcUnit(aUnits.fgh, 6, "Attacker Fighters Dog Fight")
      var d = calcUnit(dUnits.fgh, 6, "Defender Fighters Dog Fight")
      print(" ")
      aUnits.fgh -= d
      dUnits.fgh -= a
      if (aUnits.fgh < 0)
    ...
    } else if (aUnits.fgh > 0) {
      var b = calcUnit(aUnits.fgh, 6, "Attack Fighters Free Hit")
      print(" ")
      dUnits = landDamageDefender(dUnits, b)
      aUnits.fgh = 0
      dUnits.fgh = 0
    } else if (dUnits.fgh > 0) {
      //var c = calcUnit(dUnits.fgh, 6, "Defense Fighters Free Hit")
      //print(" ")
      //aUnits = landDamageAttacker(aUnits, c)
      dUnits.fgh = 0
      aUnits.fgh = 0
    }

    if (aUnits.fgh >= dUnits.fgh) {
      aAirWon = true
      dAirWon = false
      print("Attacker has Air")
      print("")
    } else {
      aAirWon = false
      dAirWon = true
      print("Defender has Air")
      print("")
    }
    return {
      aUnits: aUnits,
  ...
  // Remove all unitathat were hit by the defender
  // Order of Loss: inf, art, arm, tac, bmb
  // units - array of attacker units untis
  // hits - int value of hits casued by defenders that round
  function landDamageAttacker(units, hits) {
    // Loop through all of the hits
    while (hits > 0) {
      if (units.inf > 0) { // Inf
        units.inf -= 1;
      } else if (units.art > 0) { // Art
        units.art -= 1;
      } else if (aLandLeft) {
        if (units.arm > 1) { // Arm
          units.arm -= 1;
        } else if (units.tac > 0) { // Tac
          units.tac -= 1
        } else if (units.bmb > 0) { // Bmb
          units.bmb -= 1
        } else if (units.arm > 0) {
          units.arm -= 1
        }
      } else if (units.arm > 0) { // Arm
        units.arm -= 1;
      } else if (units.tac > 0) { // Tac
        units.tac -= 1
      } else if (units.bmb > 0) { // Bmb
  ...
  // Remove all units that were hit by the attacker
  // Order of Loss: inf, bmb, art, tac, arm
  // units - array of defender untis
  // hits - int value of hits caused by attacker that round
  function landDamageDefender(units, hits) {
    while (hits > 0) {
      if (units.inf > 0) { // Inf
        units.inf -= 1;
      } else if (units.bmb > 0) { // Bmb
  ...
  // Get the total amount of hits that the attacker scored that round
  // Take each unit type and roll an number for each unit
  // Add 1 to hits if the unit rolled less than or equal to the value
  // Return int - total maount of hits
  function landHitsAttacker(units, values) {
    var hits = 0;
    if (units.inf >= units.art && units.inf > 0) { // More inf than art
      hits += calcUnit(units.inf - units.art, values.inf, "Inf") // Inf NOT supported by art
      hits += calcUnit(units.art, values.inf + 1, "Inf w/ art"); // Inf supported by art
    } else {
      hits += calcUnit(units.inf, values.inf + 1, "Inf w/ art"); // All inf are supported by art
    }

    hits += calcUnit(units.art, values.art, "Art"); // All art

    if (aAirWon) {
      if (units.arm >= units.tac && units.arm > 0) { // More armor than tac
        hits += calcUnit(units.arm - units.tac, values.arm, "Arm"); // Arm NOT supported by tac
        hits += calcUnit(units.tac, values.arm + 1, "Arm w/ Tac"); // Arm supported by tac
      } else {
        hits += calcUnit(units.arm, values.arm + 1, "Arm w/ Tac"); // All arm supported by tac
      }
      hits += calcUnit(units.tac, values.tac, "Tac"); // All tactical
      hits += calcUnit(units.bmb, values.bmb, "Bmb"); // All bombers
    } else {
      hits += calcUnit(units.arm, values.arm, "Arm"); // All arm supported by tac
      hits += calcUnit(units.tac, dLand.tac, "Tac"); // All tactical
      hits += calcUnit(units.bmb, dLand.bmb, "Bmb"); // All bombers
    }
    print("Attack Hits: " + hits)
    print(" ")
    return hits;
  }

  // Get the total amount of hits that the defender scored that round
  // Take each unit type and roll an number for each unit
  // Add 1 to hits if the unit rolled less than or equal to the value
  // return - int - total maount of hits
  function landHitsDefender(units, values) {
    var hits = 0;
    hits += calcUnit(units.inf, values.inf, "Inf");
    hits += calcUnit(units.art, values.art, "Art");
    hits += calcUnit(units.arm, values.arm, "Arm");
    if (dAirWon) {
      hits += calcUnit(units.tac, values.tac + 2, "Tac");
      hits += calcUnit(units.bmb, values.bmb + 2, "Bmb");
    } else {
      hits += calcUnit(units.tac, values.tac, "Tac");
      hits += calcUnit(units.bmb, values.bmb, "Bmb");
    }

    print("Defense Hits:" + hits)
    print(" ")
    return hits;
  }

  // Calculate the amount of hits a unit hit that round
  // amount - int - total number of roles to do (i.e. 4 infantry would do 4 roles)
  // damage - int - number that must unit role (or lower) to score a hit
  // return = int - total amount of hits by the unit type
  function calcUnit(amount, damage, name) {
    var hits = 0;
    var i
    for (i = 0; i < amount; i++) {
      hits += roll(damage)
    ...
    if (amount > 0)
      print(name + ": " + amount + " @ " + damage + " = " + hits + " hits")
    return hits
  }
  ...
  function printUnits() {
    print("Attacker Total Units: " + aTotal)
    print(JSON.stringify(aUnits))
    print("Defender Total Units: " + dTotal)
    print(JSON.stringify(dUnits))
    print(" ");
  }
  ...
  // Get and store all of the untis in an array
  function getLand() {
    //aUnits = [parseInt($("#aInf").val()),parseInt($("#aArt").val()),parseInt($("#aArm").val()),parseInt($("#aTac").val()),parseInt($("#aBmb").val())]
    //dUnits = [parseInt($("#dInf").val()),parseInt($("#dArt").val()),parseInt($("#dArm").val()),parseInt($("#dTac").val()),parseInt($("#dBmb").val())]
